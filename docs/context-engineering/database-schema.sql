-- ============================================================================
-- INCOLAB DATABASE SCHEMA
-- Sistema de Gestión de Referencias para Laboratorio ISO 17025
-- ============================================================================

-- Eliminar tablas existentes si existen (para desarrollo/reset)
DROP TRIGGER IF EXISTS auto_create_service_tasks ON references;
...
DROP FUNCTION IF EXISTS create_service_tasks();
DROP FUNCTION IF EXISTS update_updated_at_column();

DROP TABLE IF EXISTS lab_analysis;
...
CREATE TABLE operations (
    ...
    operation_type VARCHAR(100), -- "muestreo", "inspección", "medición"
...
CREATE TABLE lab_analysis (
    ...
    analysis_type VARCHAR(100)[], -- ["humedad", "cenizas", "azufre", etc.]
...
CREATE POLICY "Referencias visibles para usuarios autenticados" ON references
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Usuarios autenticados pueden crear referencias" ON references
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Usuarios autenticados pueden actualizar referencias" ON references
...
CREATE INDEX idx_references_client_name ON references(client_name);
CREATE INDEX idx_references_reference_number ON references(reference_number);
...
-- Insertar algunas referencias de ejemplo (se ejecutará después del setup de auth)
-- Nota: Los UUIDs de usuarios deben ser reales del sistema de autenticación

/*
-- Ejemplos que se pueden insertar después del setup:

INSERT INTO references (reference_number, client_name, service_type, sample_description, location, status, notes)
VALUES
    (generate_reference_number(), 'Empresa ABC S.A.', 'both', 'Carbón bituminoso para análisis completo', 'Mina El Cerrejón, La Guajira', 'completed', 'Cliente preferencial, priorizar'),
    (generate_reference_number(), 'Industria XYZ Ltda.', 'quality_analysis', 'Biomasa pelletizada', 'Planta Industrial, Bogotá', 'in_progress', 'Requiere análisis de humedad urgente'),
    (generate_reference_number(), 'Compañía 123 S.A.S.', 'quantity_certification', 'Carbón térmico exportación', 'Puerto de Cartagena', 'pending', 'Certificación para exportación a Europa');
...
COMMENT ON TABLE references IS 'Tabla principal que almacena todas las referencias de servicios de Incolab';
...
COMMENT ON COLUMN references.reference_number IS 'Número único de referencia formato REF-YYYY-NNN';
COMMENT ON COLUMN references.service_type IS 'Tipo de servicio: análisis de calidad, certificación de cantidad, o ambos';
COMMENT ON COLUMN references.priority IS 'Prioridad del servicio para gestión de workload';
...
WHERE table_schema = 'public'
    AND table_name IN ('references', 'operations', 'lab_analysis')
ORDER BY table_name;
